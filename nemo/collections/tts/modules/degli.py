...
def str2act(txt):
    return {"sigmoid": nn.Sigmoid(), "relu": nn.ReLU(), "none": nn.Sequential() , "lrelu": nn.LeakyReLU(0.1), "selu": nn.SELU() }[txt.lower()]
...
class DeGLI_ED(nn.Module):
    ...
    def parse(self, layers:int, k_x:int, k_y:int, s_x:int, s_y:int, widening:int,use_bn: bool, linear_finalizer:bool, convGlu: bool, act: str, act2 : str, glu_bn:bool, use_weight_norm:bool) :
